...
async def Start(client, channel):
    """
    ...
    async for message in client.iter_messages(channel, min_id=start_id - 1, max_id=(end_id if end_id else None)):
        ...
        if message.media and isinstance(message.media, MessageMediaDocument):
            ...
            if not os.path.exists(file_path):
                ...
                print(f"[STATS] Total: {(total_downloaded_bytes / (1024 * 1024)):.2f} MB | Avg Speed: {avg_speed:.2f} MB/s")
            else:
    ...
    print(f"[FINAL STATS] Total Downloaded: {(total_downloaded_bytes / (1024 * 1024)):.2f} MB | Avg Speed: {avg_speed:.2f} MB/s | Total Time: {total_time:.2f} seconds")
